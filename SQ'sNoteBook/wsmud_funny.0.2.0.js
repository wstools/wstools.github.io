...
(function() {
    ...
    let listener = {
        ...
        onclose: function() {
            console.log("连接已断开");
            $("head title").html("<已掉线>");
            listener.fn_close.apply(this, arguments);
        },
            ...
            if (typeof data === "string") {
                ...
                switch (data.type) {
                    ...
                    case "state":
                        ...
                        let title = "";
                        if (funny.name) title += funny.name;
                        if (data.state) {
                            title += `<${data.state.replace("你正在", "")}>`;
                        }
    ...
    listener.addListener("text", function(message) {
        ...
        if (/重新连线|欢迎登陆/.test(data)) {
            ...
            if (funny.data_login === 1) {
                $(".content-message pre").append(`wsmud_funny ${GM_info.script.version} 苏轻祝您游戏愉快！\n`);
                getScore();
            }
    ...
    listener.addListener("msg", function(data) {
        ...
        if (data.ch === "chat") {
            ...
            if (data.content.includes("挖矿技巧")) {
                ...
                data.content.replace("，望各路英雄鼎力相助", "");
            }
    ...
    listener.addListener("dialog", function(data) {
        if (data.dialog === "score") {
            ...
            let name = funny.role.name;//普通百姓 峨眉苏轻<hig>&lt;学习基本招架&gt;</hig>
            name = name.replace(/<|>|\/|wht|hig|hiy|hio|(.*)\s|&lt;(.*)&gt;/g, "");
            funny.name = name;
        }
    });
    // skills
        ...
        if (data.dialog === "skills") {
            ...
            if (data.items) {
                ...
                if (funny.state === "你正在练习技能") {
                    ...
                    fn.scroll(".content-message");
                } else if (funny.state === "你正在读书" || /你正在学习(.*)/.test(funny.state)) {
    ...
    if (WebSocket) {
        unsafeWindow.WebSocket = function(uri) {
            funny.webSocket = new WebSocket(uri);
        };
        unsafeWindow.WebSocket.prototype = {
            ...
            set onclose(fn) {
                listener.fn_close = fn;
                funny.webSocket.onclose = listener.onclose;
    ...
    $(document).ready(function() {
        ...
        $(".left").append(
            ...
            $(`<div class="left-hotkey left-hide"></div>`).append(
                $(`<div>小号常用的快捷键</div>`),
                $(`<div class="item-commands"></div>`).append(
                    $(`<span>送给配偶99朵玫瑰</span>`).click(() => fn.send("greet 99")),
                ),
            ),
